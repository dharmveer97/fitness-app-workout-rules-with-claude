...
// Enum schemas
export const WorkoutTypeSchema = z.enum(['strength', 'cardio', 'yoga', 'hiit', 'pilates', 'running', 'cycling', 'swimming', 'other']);

export const WorkoutDifficultySchema = z.enum(['beginner', 'intermediate', 'advanced']);

export const ActivityTypeSchema = z.enum(['workout', 'meal', 'water', 'sleep', 'steps', 'weight', 'heart_rate']);
...
export const UserGoalsSchema = z.object({
  dailySteps: z.number().int().min(1000, 'Daily steps goal must be at least 1000').max(50000, 'Daily steps goal cannot exceed 50000'),
  dailyWater: z.number().min(500, 'Daily water goal must be at least 500ml').max(5000, 'Daily water goal cannot exceed 5000ml'),
  dailyCalories: z.number().min(1200, 'Daily calories goal must be at least 1200').max(5000, 'Daily calories goal cannot exceed 5000'),
  weeklyWorkouts: z.number().int().min(1, 'Weekly workouts goal must be at least 1').max(14, 'Weekly workouts goal cannot exceed 14'),
  sleepHours: z.number().min(6, 'Sleep hours goal must be at least 6').max(12, 'Sleep hours goal cannot exceed 12'),
...
export const UserProfileSchema = z.object({
  ...
  age: z.number().int().min(13, 'Age must be at least 13').max(120, 'Age cannot exceed 120').optional(),
...
export const AchievementSchema = z.object({
  ...
  category: z.enum(['workout', 'streak', 'calories', 'steps', 'water', 'sleep']),
});
...
export const UpdateProfileSchema = UserProfileSchema.omit({ id: true, joinDate: true }).partial();
